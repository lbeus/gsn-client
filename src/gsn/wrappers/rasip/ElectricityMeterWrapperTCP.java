...
import gsn.wrappers.*;
import gsn.electricityTCP.*;

public class ElectricityMeterWrapperTCP extends AbstractWrapper{
    ...
    private transient DataField [ ] outputStructureCache = new DataField[] { new DataField( FIELD_CURRENT_POWER_T1, "double", "Current power tarrife one" ), new DataField( FIELD_CURRENT_POWER_T2, "double", "Current power tarrife two" ),
            new DataField( FIELD_MAX_CUMM_POWER_T1, "double" , "Maximum cummulative power tarrife one." ), new DataField( FIELD_MAX_CUMM_POWER_T2, "double" , "Maximum cummulative poweer tarrife two" ), new DataField( FIELD_POWER, "double", "Electricity Power" ),
            new DataField( FIELD_POWER_T1, "double", "Electricity Power tarrife one" ), new DataField( FIELD_POWER_T2, "double", "Electricity Power tarrife two" ), new DataField( CURRENT_PHASE_1, "double", "Current phase 1" ),new DataField( CURRENT_PHASE_2, "double", "Current phase 2" ),new DataField( CURRENT_PHASE_3, "double", "Current phase 3" )};
    
    private static final String [ ] FIELD_NAMES = new String [ ] { FIELD_CURRENT_POWER_T1, FIELD_CURRENT_POWER_T2, FIELD_MAX_CUMM_POWER_T1, FIELD_MAX_CUMM_POWER_T2, FIELD_POWER, FIELD_POWER_T1, FIELD_POWER_T2, CURRENT_PHASE_1, CURRENT_PHASE_2, CURRENT_PHASE_3 };
    
	
	double current_power_t1, current_power_t2, max_cumm_power_t1, max_cumm_power_t2, power, power_t1, power_t2, current_phase1, current_phase2, current_phase3;
	
    public boolean initialize() {
        logger.info("Initializing ElectricityMeterWrapperTCP Class");
        ...
        setName("ElectricityMeterWrapperTCP-Thread" + (++threadCounter));
    ...
    public void run(){
        while(isActive()){
            ...
            StreamElement streamElement = new StreamElement( FIELD_NAMES , new Byte [ ] { DataTypes.DOUBLE, DataTypes.DOUBLE, DataTypes.DOUBLE, DataTypes.DOUBLE, DataTypes.DOUBLE, DataTypes.DOUBLE, DataTypes.DOUBLE,DataTypes.DOUBLE,DataTypes.DOUBLE,DataTypes.DOUBLE } , new Serializable [ ] { current_power_t1, current_power_t1,
            		max_cumm_power_t1, max_cumm_power_t2, power, power_t1, power_t2, current_phase1, current_phase2, current_phase3 }, System.currentTimeMillis( ) );
